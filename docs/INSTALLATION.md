...
Questa guida descrive il pacchetto Windows di Obsidian Bridge 0.5.8. Il flusso normale non richiede PowerShell, modifica di file JSON o variabili d'ambiente.

L'interfaccia WPF dell'installer è adattiva: usa righe automatiche, manda a capo percorsi e messaggi lunghi e mantiene raggiungibili i comandi tramite scorrimento anche con finestre compatte o ridimensionamento elevato di Windows.
...
Per i vault configurati tramite Bridge Control, ogni osservazione create/append usa un unico percorso UTF-8 esatto basato sulle impostazioni: prepare, controllo conflitto al commit, backup append, blocchi intermedi, verifica finale e classificazione del recupero. Questo accesso al filesystem è di sola lettura; le mutazioni continuano a usare soltanto la CLI ufficiale allowlistata. Ogni proposta create/append accetta al massimo 64 KiB di contenuto UTF-8 e la relativa anteprima completa è limitata a 192 KiB. Il documento risultante dopo append resta limitato a 1 MiB e create richiede che la cartella padre della destinazione esista già. I contenuti lunghi continuano a essere suddivisi in frame CLI completi di non più di 3072 byte UTF-8; il bridge non crea cartelle implicitamente.
