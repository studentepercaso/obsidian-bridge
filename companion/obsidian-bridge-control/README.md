...
Bridge Control is the desktop-only Obsidian companion for Obsidian Bridge. Version 0.5.5 removes all child-process execution from the companion, addresses the current Obsidian review findings, and preserves the explicit **Full management**, Protected, and Autonomous profiles without adding permissions or mutation capabilities.
...
The matching 0.5.5 bridge uses settings-backed bounded exact UTF-8 snapshots for every create/append and managed transactional observation, including prepare, CAS, backup capture, chunk/final verification, and recovery classification. The snapshot path is read-only; create/append mutation remains on the allowlisted official Obsidian CLI. Each create/append proposal accepts at most 64 KiB of UTF-8 content, its complete preview is bounded to 192 KiB, and long content remains split into complete CLI frames of at most 3072 UTF-8 bytes. Environment-only create/append requires migration to Bridge Control, the resulting appended document remains bounded to 1 MiB, and create requires an existing parent folder. Version 0.5.5 changes neither the command protocol nor managed mutation code.
...
Bridge Control 0.5.5 atomically maintains strict shared-settings version 5:
...
Then reload Obsidian and enable **Bridge Control** under Community plugins. Managed operations require Obsidian 1.12.7 or later, the official CLI enabled for the external bridge, and the matching Obsidian Bridge 0.5.5 release.
