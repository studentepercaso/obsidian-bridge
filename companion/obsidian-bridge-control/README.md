...
Bridge Control is the desktop-only Obsidian companion for Obsidian Bridge. Version 0.5.8 coordinates with the matching bridge while preserving the synchronized-read fix, the review-hardened companion, the explicit **Full management**, Protected, and Autonomous profiles, and the existing permission boundaries.
