...
- A guided, adaptive WPF Windows installer with vault discovery, a self-generated verified local marketplace, readable high-DPI layouts, and no administrator privileges.
...
- Cross-process commit locks, at most 20 shared local plaintext recovery backups, postcondition verification, content-free audit metadata, a **Recent problems** panel, and direct bounded audit diagnostics for Codex after an error. Version 0.5.8 accepts up to 64 KiB in one create/append proposal and verifies synchronized notes with repeated byte-identical reads plus a bounded final stable metadata window, avoiding false conflicts from `ctime`-only Windows metadata updates while real content changes still fail closed.

## Quick start on Windows

1. Download **Obsidian-Bridge-Setup-0.5.8.zip** from the [releases page](https://github.com/studentepercaso/obsidian-bridge/releases).
...
Use the asset whose name starts with **Obsidian-Bridge-Setup**. GitHub's automatically generated **Source code** archives are development snapshots, not the guided installer. SHA-256 values are published beside every release in **SHA256-0.5.8.txt**.

The 0.5.8 installer and Bridge Control interface are currently in Italian; the English guide maps each step. Its WPF layout wraps long paths and instructions, keeps the action area reachable through scrolling, and adapts to compact windows and Windows display scaling.
...
- [0.5.8 bilingual release notes](docs/RELEASE_NOTES_0.5.8.md)
- [Privacy](PRIVACY.md)
