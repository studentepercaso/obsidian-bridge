...
3. Prepare the smallest change with `obsidian_prepare_change`. Pass a vault-relative Markdown path, operation, and 1-8192 UTF-8 bytes of proposed content. Reject content containing the literal backslash sequences `\n` or `\t`; the official CLI cannot represent them losslessly. Do not alter other backslashes.
...
7. After success, check the commit's `verified`, optional `backup_id`, and `audit_recorded` fields. Read the affected note back and report the verified result. Cite resulting lines when available. Explain that append backups contain plaintext prior content. If verification fails, report `rollback_attempted`, `rollback_succeeded`, and `rollback_reason` exactly and never imply the vault is unchanged without rereading it. Automatic rollback is limited to one overwrite when the original is CLI-representable and at most 8192 UTF-8 bytes; otherwise explain manual recovery from the backup when the reason is `restore_unrepresentable` or `restore_too_large`.
